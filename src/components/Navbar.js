...
const Navbar = () => {
  ...
  return (
    <header className="text-white body-font font-poppins fixed inset-x-0 z-50 bg-darkblue">
      <div className="mx-auto py-2 px-5 flex flex-row justify-between lg:justify-around items-center lg:py-3">
        ...
        >
          <img
            className="w-20 h-15 ml-3 cursor-pointer scale-125 lg:scale-150"
        ...
        <nav
          ...
          data-aos-duration="1000"
          data-aos-once="false"
          className="hidden md:mx-auto lg:flex flex-wrap items-center text-[1.35rem]  justify-center gap-12"
        >
          {Navlinks.map((item) => {
            return (
              <Link
        ...
        {isMenuOpen && (
          <div className="min-h-[calc(100vh-4rem)] absolute inset-x-0 top-16 z-50 transition-all ease-in duration-1000 lg:hidden">
            ...
            >
              <nav className="flex flex-col gap-y-7 text-2xl font-semibold">
                {Navlinks.map((item) => {
                  return (
                    <Link
                      key={item.title}
